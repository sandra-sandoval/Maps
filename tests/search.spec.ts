...
test("searching through multiple files", async ({ page }) => {
  ...
  await page.getByPlaceholder("Enter command here!").press("Enter");
  await expect(page.locator("table")).toBeHidden;
  await expect(page.getByRole("paragraph").nth(0)).toContainText(
  ...
  await page.getByPlaceholder("Enter command here!").press("Enter");
  await expect(page.getByRole("paragraph").nth(6)).toContainText(
    "Output: No data to display"
  );
  await page.getByLabel("Command Input Box to type in commands").fill("view");
  await page.getByPlaceholder("Enter command here!").press("Enter");
  ...
  await expect(page.getByRole("paragraph").nth(14)).toContainText(
