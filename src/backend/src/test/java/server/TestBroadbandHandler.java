...
/**
 * Integration tests: send real web requests to our server as it is running.
 * Note that for these, we
 * prefer to avoid sending many real API requests to the NWS, and use "mocking"
 * to avoid it. (There
 * are many other reasons to use mock data here. What are they?)
 *
 * <p>
 * In short, there are two new techniques demonstrated here: integration
 * testing; and testing
...
public class TestBroadbandHandler {
  ...
  /**
   * Test case to verify a successful /broadband request with an echo mock
   * CensusSource. Checks the
   * accuracy of the returned broadband access percentage (set - 80.0%) and the
   * time of data
   * retrieval.
   *
  ...
  /**
   * Test case to verify a successful /broadband request using a stale mock
   * CensusSource. Checks
   * whether the response body contains the expected broadband access percentage
   * (values differ
  ...
  /**
   * Test case to verify the handling of a /broadband request with a missing
   * "state" parameter using
   * an echo mock CensusSource. Checks whether the response body contains an error
   * result and
   ...
   * Test case to verify the handling of a /broadband request with a missing
   * "county" parameter
   * using a stale mock CensusSource. Checks whether the response body contains an
   * error result and
   * message indicating the missing "state" parameter.
   *
   * @throws IOException if an I/O error occurs while making the HTTP request
   ...
   * Test case to verify the handling of a /broadband request with a missing
   * "state" parameter using
   * a stale mock CensusSource. Checks whether the response body contains an error
   * result and
   * message indicating the missing "county" parameter.
   *
   * @throws IOException if an I/O error occurs while making the HTTP request
  ...
  /**
   * Test case to verify the handling of a /broadband request with an invalid
   * "state" parameter.
   * Checks whether the response body contains an error result indicating the
   * invalid "state"
   * parameter.
   *
   * @throws IOException if an I/O error occurs while making the HTTP request
  ...
  /**
   * Test case to verify the handling of a /broadband request with empty "state"
   * and "county"
   * parameters. Checks whether the response body contains an error result
  ...
  /**
   * Test case to verify the caching behavior of the BroadbandHandler when a
   * CacheBuilder is
   * provided. Ensures that the cache is of the same size after sending requests
   * with equal
  ...
  public void testBroadbandCache() throws IOException {
    ...
    Map<String, Object> body1 = adapter.fromJson(new Buffer().readFrom(connection1.getInputStream()));
    ...
    Map<String, Object> body2 = adapter.fromJson(new Buffer().readFrom(connection2.getInputStream()));
    ...
    Map<String, Object> body3 = adapter.fromJson(new Buffer().readFrom(connection3.getInputStream()));
  ...
  /**
   * Test case to verify the behavior of the BroadbandHandler when no CacheBuilder
   * is provided,
