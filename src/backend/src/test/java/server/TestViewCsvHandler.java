 ...
 *
 * <p>
 * This is a testing class that tests the ViewCsvHandler class. ViewCsvHandler
 * handles /viewcsv
 * requests to our Server. This class tests many cases for how combiantions of
 * /loadcsv and /viewcsv
 * can occur, as well as different kinds of files loaded, missing parameters,
 * and more.
 *
 * @version 1.0
 */
public class TestViewCsvHandler {
  ...
  /**
   * This method reinitializes the /loadcsv and /viewcsv endpoints, and well as
   * the handler, csvData
  ...
  /**
   * This test verifies that if /viewcsv is called without any call to /loadcsv
   * occuring, that the
   * handler returns a failure response object with a proper error code and
   * message.
   *
  ...
  /**
   * This test verifies that if /loadcsv is called, but fails, and then /viewcsv
   * is called, that the
   * view handler returns a failure response object with a proper error code and
   * message.
   *
   * @throws IOException if the CSV parsing done by the handler's call to
   *                     CsvParser fails
   ...
   * different file is
   * loaded with /loadcsv, that when /viewcsv is called, the second file loaded is
   * viewed.
   *
   * @throws IOException if the CSV parsing done by the handler's call to
   *                     CsvParser fails
  ...
  public void testLoadFirstLoadSecondView() throws IOException {
    ...
    assertEquals(200, loadConnection.getResponseCode());
    Map<String, Object> loadSecondBody = adapter.fromJson(new Buffer().readFrom(loadSecondConnection.getInputStream()));
  ...
  /**
   * This test verifies that if /loadcsv is called to load an empty CSV file, and
   * then /viewcsv is
   * called, that the view handler returns a success object with a data field
   * containing an empty
   * list of rows.
   *
   * @throws IOException if the CSV parsing done by the handler's call to
  ...
  /**
   * This test verifies that if /loadcsv is called to load a full CSV file, and
   * then /viewcsv is
   * called, that the view handler returns a success object with a data field
   * containing all the
