...
/**
 * This LoadCsvHandler class handles a /loadcsv request to our server. It is
 * constructed with a
 * CsvDataWrapper object, wrapping a List of List of String representing the CSV
 * data. If the
 * filepath passed as a query parameter is found, the handle method parses the
 * CSV and stores its
 * rows in the CsvDataWrapper object and sets the wrapper's loadedInPast field
 * to true.
 */
public class LoadCsvHandler implements Route {
   ...
   *
   * @param csvData a CsvDataWrapper object, with fields csvData, representing
   *                rows of loaded CSV
   *                data, and loadedInPast, a boolean representing whether a CSV
   *                has been loaded
   */
  ...
  /**
   * Method that handles a /loadcsv request to our Server. Given a request and
   * response, returns a
   * failure or success response object. If successful, the CSV data in the
   * filepath is loaded in
   * CsvDataWrapper and its loadedInPast field is set to true. A load is only
   ...
   * @return response object depending on whether the file loading and parsing was
   *         successful
   */
  @Override
  public Object handle(Request request, Response response) {
    ...
    try {
      ...
      // if (!(paths[0].equals("repl") && paths[1].equals("src") &&
      // paths[2].equals("backend") && paths[4].equals("data"))) {
      // return new LoadFailureResponse(
      // "error_datasource", "Filepath located in an inaccessible directory",
      // csvFilePath)
      // .serialize();
      // }
      String fullFilePath = directory + "/src/backend/src/main/java/" + csvFilePath;
  ...
  /**
   * A record representing a failed call to the /loadcsv handler, containing a
  ...
  /**
   * A record representing a successful call to the /loadcsv handler, containing a
   * result of
