...
/**
 * This ViewCsvHandler class handles a /viewcsv request to our server. It is
 * constructed with a
 * CsvDataWrapper object, wrapping a List of List of String representing the CSV
 * data. If there is
 * data loaded in the CsvDataWrapper object, then this request returns an object
 * with a field
...
public class ViewCsvHandler implements Route {
  ...
  /**
   * Method that handles a /viewcsv request to our Server. Given a request and
   * response, returns a
   * failure or success response object. If successful, the response object
   * contains all the rows of
   * CSV data in the CSV file loaded.
   *
   * @param request  the Request object passed by client, which needs no query
   *                 parameters
  ...
  /**
   * A record representing a failed call to the /viewcsv handler, containing a
  ...
  /**
   * A record representing a successful call to the /viewcsv handler, containing a
   * result of
