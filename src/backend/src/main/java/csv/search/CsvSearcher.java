...
/**
 * This class is the CsvSearcher class, which is called by the main run
 * function. Constructed with a
 * CsvParser object, an object of the CsvSearcher class can search the csvRows
 * from the CsvParser's
 * methods parseCsv and getStoreRows to find rows with the given search
 * criteria.
 */
public class CsvSearcher {
  ...
  /**
   * A method that uses the String colID to find the numeric index of the target
   * column, only called
   * if there is a specific column to be searching. CsvRows is guaranteed to not
   * be empty because we
   ...
   * @param colId      String representing the name or index of column to be
   *                   searched
   * @return integer representing the index of column to be searched
   * @throws IndexOutOfBoundsException if the target column is not found or is out
   *                                   of range
   */
  ...
  /**
   * A method that searches all columns of each row of the CSV for the desired
   * searchValue. NOTE:
   * the searchValue much match an item in the row exactly (case matters).
   *
   * @param searchValue String representing the value to find in any column of a
   *                    row
   * @return List of String containing the rows where the searchValue is found
   *         exactly
   */
  ...
  /**
   * A method that searches only one column, with index colIndex, and returns all
   * rows where
   * searchValue is found in the column with that index.
   *
   * @param searchValue String representing the value to be found in its exact
   *                    form
   * @param colIdIsNum  boolean representing whether column ID is numeric
   * @param colId       String which the method includes in an error message if
   *                    exception is thrown
   * @return List of String of rows where searchValue in its exact form is found
   *         in the column with
  ...
  private List<List<String>> searchOneCol(
    ...
    try {
      ...
      // For each row, get the item at the colIndex and check if it matches
      // searchValue
      for (List<String> row : csvRows.subList(start, csvRows.size())) {
  ...
  /**
   * A public method, which is called by the Main runner, which searches for
   * searchValue in the
   * column(s) specified by colID, and returns a list of matching rows or a list
   * containing an error
   * message describing what went wrong.
   *
   * @param hasHeaders  boolean representing whether the CSV has a row of headers
   * @param searchValue String representing the value to be matched exactly in the
   *                    CSV data
   * @param colId       String representing the column(s) to search for the exact
   *                    appearance of
   *                    searchValue
   * @return List of String containing the matching rows or only containing an
   *         error String
