...
import sources.DataSourceException;
import java.util.HashMap;
import java.util.List;

/**
 * The StaleMockCensusSource class implements the CensusSource interface and
 * overrides its
 * getBroadbandAccess method. An object of this class is constructed with
 * nothing, but in the
 * constructor, the field broadbandAccess is filled with stale ACS broadband
 * data from 9/25/23 in
 * North Carolina and Rhode Island. When the getBroadbandAccess method is
 * called, an object of this
 * class returns the stale broadband access data if the given state and county
 * combination exists in
 * North Carolina or Rhode Island. Otherwise, this source returns a
 * DataSourceException.
 *
 * This mock class enables us to test our /broadband handler without making many
 * repeated calls
 * to an object of the AcsCensusSource class, which makes real requests to the
 * census API.
 */
public class StaleMockCensusSource implements CensusSource {
  ...
  /**
   * The constructor for the StateMockCensusSource class, which fills the
   * broadbandAccess field with
   * stale data from 9/25/23 for North Carolina and Rhode Island counties.
   */
  public StaleMockCensusSource() {
    List<List<String>> stateData = List.of(
  ...
  /**
   * A method that returns a CensusData object representing the stale broadband
   * access level from
   * 9/25/23 for a state / county combination in North Carolina or Rhode Island.
   * Otherwise, throws a
   * DataSourceException.
   *
   * @param state  the String representing the state query parameter from the
   *               handler
   * @param county the String representing the county query parameter from the
   *               handler
   * @return CensusData object containing a Double representing the stale
   *         broadband access level
   * @throws DataSourceException if the state / county combination does not exist
   *                             in NC or RI
   */
  @Override
